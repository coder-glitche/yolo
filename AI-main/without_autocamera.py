...
desired_targets = [
    {'shape': 'Circle', 'color': 'white', 'letter': 'A', 'id': 1},
    {'shape': 'Quartercircle', 'color': 'green', 'letter': '6', 'id': 2},
    {'shape': 'Triangle', 'color': 'red', 'letter': '9', 'id': 3},
    {'shape': 'Pentagon', 'color': 'brown', 'letter': '4', 'id': 4},
    {'shape': 'Circle', 'color': 'white', 'letter': 'R', 'id': 5}
...
for key, value in defaults.items():
    if key in ['grid','suas','alt','fov','lap', 'test', 'sitl', 'ros', 'resume']:
...
if not resume_en:
    try:
        shutil.rmtree('/home/edhitha/ultralytics/yo/AI/work/output1/test4')
    except:
        pass

    try:
        shutil.rmtree('/home/edhitha/ultralytics/yo/AI/work/output2/test5')
    except:
        pass

    try:
        shutil.rmtree('/home/edhitha/ultralytics/yo/AI/work/output3')
    except:
        pass
    try:
        os.makedirs( '/home/edhitha/ultralytics/yo/AI/work/log_results', exist_ok=True)
        shutil.move('/home/edhitha/ultralytics/yo/AI/work/output1/test4', '/home/edhitha/ultralytics/yo/AI/work/output1/log_results')
...
# model1 = YOLO('/home/edhitha/ultralytics/runs/detect/emergent_target_model/weights/best.pt')
model1 = YOLO('/home/edhitha/ultralytics/runs/detect/train52/weights/best.pt')
print('Time required to load the first model:', time.time() - start_time)
model2 = YOLO('/home/edhitha/ultralytics/runs/classify/Model_colored_mask/weights/best.pt')
print('Time required to load the second model:', time.time() - start_time)
model4 = YOLO('/home/edhitha/ultralytics/runs/segment/latest_shape/best.pt')


if ros_en:
    input_folder = '/home/edhitha/DCIM/test_cam/images'
    # input_folder = '/home/edhitha/Images_4_3_24'
else:
    input_folder = '/home/edhitha/DCIM/test_cam/images'
output_folder = '/home/edhitha/ultralytics/yo/AI/work/output1'
output_folder2 = '/home/edhitha/ultralytics/yo/AI/work/output2'
output_dir = "/home/edhitha/ultralytics/yo/AI/work/output3"
Path(output_dir).mkdir(parents=True, exist_ok=True)
results_file = '/home/edhitha/ultralytics/yo/AI/work/results.txt'
results_csv_file = '/home/edhitha/ultralytics/yo/AI/work/output1/test4/crops/results/results.csv'
modified_csv_file = '/home/edhitha/ultralytics/yo/AI/work/output1/test4/crops/results/results.csv'
ros_csv_file_path = "/home/edhitha/ultralytics/yo/AI/results.csv"
eliminate_csv_file = '/home/edhitha/ultralytics/yo/AI/work/output1/test4/crops/Target/detected.csv'
...
os.makedirs('/home/edhitha/ultralytics/yo/AI/work/output1/test4/crops/Target', exist_ok=True)
os.makedirs('/home/edhitha/ultralytics/yo/AI/work/output1/test4/crops/results', exist_ok=True)
# Pickle path
...
def execute_with_timeout(func, timeout2, *args, **kwargs):
...
def drop(index):
    ...
    pwm_values = [(9,1858),(9,1142),(10,2012),(10,1019),(11,1193)]
...
def extract_data_for_file(file_path, target_file):
...
def set_mission_waypoint(master, waypoint_number):
    # Construct the COMMAND_LONG message to set the mission waypoint
    msg = master.mav.command_long_encode(
        master.target_system,       # Target system ID
        master.target_component,    # Target component ID
        mavutil.mavlink.MAV_CMD_DO_SET_MISSION_CURRENT,  # Command
        0,                          # Confirmation
        waypoint_number,            # Param1: Mission sequence value to set
        0,                          # Param2: Reset Mission (not used)
        0, 0, 0, 0, 0                # Param3 to Param7: Empty
    )
    # Send the COMMAND_LONG message
    master.mav.send(msg)

def restart_mission(master):
    # Call set_mission_waypoint with waypoint_number = 0 to resume the mission
    set_mission_waypoint(master, 0)
...
def get_wps_and_indicies(vehicle):
    ...
    try:
        ...
        wp_jump_index = wp_last_occurrence + 1
        search_jump_index = commands_df[commands_df['Command'] == 177].index[1]
    except Exception as e:
        print("ERROR, NO DO_JUMP COMMAND FOUND", e)
    ...
    # Print the result
    print(f"wp_last_occurrence: {wp_last_occurrence}, search_wp_last_occurrence: {search_wp_last_occurrence}")

    return commands_df.loc[wp_last_occurrence], commands_df.loc[search_wp_last_occurrence], wp_last_occurrence, search_wp_last_occurrence, commands_df, wp_jump_index, search_jump_index
...
if not lap_en:
    ...
    try:
        wp_row, search_wp_row, wp_index, search_index, commands_df, wp_jump_index, search_jump_index = get_wps_and_indicies(the_connection)
    except Exception as e:
        ...
        if image_path not in processed_images:
            if grid or alt_limiter:
                try:
                    image_index = int(image_file[-9:-4])
                    if ros_en:
                        lat1, lon2, alt, head, yaw, time1 =  extract_data_for_file(ros_csv_file_path, image_file)
                    else:
                ...
                for j, box in enumerate(result.boxes):
                    ...
                    if class_name == 'Emergent':
                        ...
                        if shapes:
                            ...
                            results2 = model2.predict(source=output_path, project=output_folder2, name='test5', device=0,exist_ok=True, save=True)
                            ...
                            try:
                                while lap in read_df['ID'].values:
                                    ...
                                    if row['Dropped'].astype(bool) == False:
                                        ...
                                        print("Giving wp jump index:", wp_jump_index)
                                        set_mission_waypoint(the_connection, wp_jump_index)
...
if resume_en:
    ...
    try:
        emergent_files = os.listdir(Emergent_folder) 
        emergent_image_files = [f for f in emergent_files if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
        for x in emergent_image_files:
            source_path = os.path.join(Emergent_folder, x)
            print(source_path)
            target_path = os.path.join(Target_folder, x)
