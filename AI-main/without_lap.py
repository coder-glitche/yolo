...
for key, value in defaults.items():
    if key in ['grid','suas','alt','fov','lap', 'test']:
...
model1 = YOLO('/home/edhitha/ultralytics/runs/detect/targetmodel_16mm/weights/best.pt')
...
input_folder = '/home/edhitha/DCIM/test_cam_190/images'
...
# Define your list of desired targets
desired_targets = [
    {'shape': 'Quartercircle', 'color': 'purple', 'letter': 'x', 'id': 1},
    {'shape': 'Semicircle', 'color': 'green', 'letter': 'm', 'id': 2},
    {'shape': 'Quartercircle', 'color': 'blue', 'letter': '3', 'id': 3},
    {'shape': 'Star', 'color': 'black', 'letter': '1', 'id': 4},
    {'shape': 'Star', 'color': 'orange', 'letter': '8', 'id': 5},
    {'shape': 'Cross', 'color': 'white', 'letter': 'j', 'id': 6}
...
def drop(index):
    ...
    pwm_values = [1720,1350,1050,1000,1900]
    # Set the initial PWM value
    pwm_value = pwm_values[index]
    print(pwm_value)
...
def connect():
    ...
    global the_connection
    #the_connection = mavutil.mavlink_connection('udp:127.0.0.1:14553')
...
def get_wps_and_indicies(vehicle):
    ...
    return commands_df.loc[wp_last_occurrence], commands_df.loc[search_wp_last_occurrence], wp_last_occurrence, search_wp_last_occurrence
...
if lap_en:
    ...
    try:
        wp_row, search_wp_row, wp_index, search_index = get_wps_and_indicies(the_connection)
    except Exception as e:
            ...
            for i, result in enumerate(results):
                for j, box in enumerate(result.boxes):
                    ...
                    if class_name_counts.get(class_name, 0) > 0:
                        crop_name = f"{image_file[:-4]}{class_name_counts[class_name]}.jpg"
                    else:
                        crop_name = f"{image_file[:-4]}.jpg"

                    print(crop_name)
                    crop_path = os.path.join(output_folder, 'test4', 'crops', 'Target', crop_name)
                    lat = value[0] / 1e7
                    lon = value[1] / 1e7
                    ...
                    eliminate_df = pd.concat([eliminate_df, pd.DataFrame({
                        ...
                        'Lat': lat,
                        'Lon': lon,
                        'Cropped Image Path': [os.path.join(output_folder, 'test4', 'crops', 'Target', crop_name)]
                    })], ignore_index=True)
                    print( crop_path)
                    ...
                    for i, result in enumerate(results4):
                        for j, box in enumerate(result.boxes):
                            shape_name = result.names[box.cls[0].item()]
                            # print(class_name)
                    ...
                    if check:
                        ...
                        if value:   
                            ...
                            shutil.copy(os.path.join(output_folder, 'test4', 'crops', 'Target', crop_name),'/home/edhitha/ultralytics/yo/output1/test4/crops/results')
                        else:
        ...
        if changes > 10:
            ...
            for i in range(num_targets - 1):
                ...
                if lap_en and i != 0:
                    while True:
                        if i != 0:
                            print('Getting Drone global coordinates and Mission Item')
                            msg = the_connection.recv_match(type=['GLOBAL_POSITION_INT', 'MISSION_CURRENT'], blocking=True)
                            if msg.get_type() == 'MISSION_CURRENT':
                                print('Current mission item:', msg.seq)
                                time.sleep(0.2)
                                if 3 + 1 != msg.seq and msg.seq != prev_item:
                                    time.sleep(0.2)
                                    prev_item = msg.seq
                                    continue
                                else:
                ...
                Lat = read_df['Lat'][i]
                Lon = read_df['Lon'][i]
                print('REPOSITIONING TO COLOR:',read_df['Color'][i])
                print('REPOSITIONING TO LETTER:',read_df['Letter'][i])
                print('REPOSITIONING TO SHAPE:',read_df['Object Type'][i])
                automation((Lat, Lon), i)
                time.sleep(4)
...
if test_en:
    ...
    for i in range(num_targets - 1):
        # print(read_df.loc[i])
        row = read_df.loc[read_df['ID'] == i + 1]
        ...
        if lap_en and i != 0:
            while True:
                if i != 0:
                    ...
                    break
        Lat = row['Lat'].astype(float).iloc[0]
        Lon = row['Lon'].astype(float).iloc[0]
